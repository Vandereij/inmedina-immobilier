...
export async function POST(req: Request) {
  try {
    ...
    if (!SUPABASE_URL.startsWith("https://") || !SUPABASE_URL.includes(".supabase.co")) {
      throw new Error("NEXT_PUBLIC_SUPABASE_URL looks wrong (must be https://<ref>.supabase.co)");
    }
    if (SERVICE_ROLE.length < 50) throw new Error("Service role key looks too short.");

    const body = await req.json();

    // allowlist the fields you expect
    const allowed = [
      "title","slug","availability","price","cover_image_url",
    ...
    const { error, data } = await admin.from("properties").insert(payload).select("*");
    if (error) {
